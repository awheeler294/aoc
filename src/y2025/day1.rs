...
fn solve_part_1(input: &[&str]) -> u32 {
   fn solve(input: &[&str], dial_position: i32) -> u32 {
      ...
      let n: i32 = if let Some(n) = input[0].strip_prefix('L') {
         -1 * n.parse::<i32>().unwrap_or_else(|e| panic!("unable to parse `{}`: {e}", input[0]))
      } else if let Some(n) = input[0].strip_prefix('R') {
         n.parse::<i32>().unwrap_or_else(|e| panic!("unable to parse `{}`: {e}", input[0]))
      } else {
...
fn solve_part_2(input: &[&str]) -> i32 {
   ...
   for line in input {
      let distance: i32 = if let Some(n) = line.strip_prefix('L') {
         -1 * n.parse::<i32>().unwrap_or_else(|e| panic!("unable to parse `{}`: {e}", line))
      } else if let Some(n) = line.strip_prefix('R') {
         n.parse::<i32>().unwrap_or_else(|e| panic!("unable to parse `{}`: {e}", line))
      } else {
         panic!("malformed input `{}`", input[0]);
      };
